...
class reg: public Component {
    ...
    public:
        reg(Netlist *netlist, std::string line, int a){
            ...
            std::vector<Variable> inputs = this->netlist->getInputs();
            bool hasClkRst = false;
            ...
            if(!hasClkRst){
                this->netlist->addVariable("input UInt1 clk");
                this->netlist->addVariable("input UInt1 rst");
            }
        ...
        void checkRegisters() override {
            ...
            for(std::shared_ptr<Component> reg : regs){
                ...
                for(std::shared_ptr<Component> elem : regs){
                    for(unsigned i = 0; i < elem->getInputs().size(); i++){
                        if(this->inputs[0] == elem->getInputs()[i]){
                            elem->setInput(i, reg->getInputs()[0]);
                        }
                    }
